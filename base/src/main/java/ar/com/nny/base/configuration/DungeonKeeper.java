...
public class DungeonKeeper {
    ...
    public CodeBlock buildInvocationChain(final Object service, final Method method, final Object[] args, final MethodProxy methodProxy) {
        if ( this.excludedMethods.contains(method.getName()) ) {
           ...
           // return new SynchronicityBlock(new ContextSessionBlock(new ExceptionManagementBlock(new SecurityBlock(new TransactionalBlock(new DataFilterBlock(new HookedInvocationBlock(service, args, method, methodProxy)))))));
        }
		return null;
    }
